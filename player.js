...
(() => {
  ...
  const CW_KEY = 'mjplus_cw_v17'; // bump to v17 wegen neuer Meta-Felder
  ...
  const upsertCW = (entry) => {
    ...
    // nur Felder speichern, die wir wirklich brauchen
    const clean = {
      id: entry.id,
      title: entry.title,
      poster: entry.poster,
      source: entry.source || '',
      mp4: entry.mp4 || '',
      t: Math.max(0, entry.t|0),
      d: Math.max(0, entry.d|0),
      // NEU: Resume-Metadaten für Bundles/Varianten
      recKey: entry.recKey || null,
      verKey: entry.verKey || null
    };
  ...
  // Liefert ein „spielbares“ Objekt (id/title/poster/type/source/mp4/chapters) aus versions[]
  ...
  // Bundle → konkretes Play-Objekt aus recording + version
  ...
  /* ---------- Setlist anzeigen (Recording + Variantenschalter) ---------- */
  function showSetlist(item){
    ...
    // Variantenschalter (Bundle: aus Recording; sonst: aus item.versions/variants)
    function mountVariantSwitch(){
      ...
      if (isBundle(item)){
        ...
        variants = rec?.versions || [];
        if (!currentVariant) currentVariant = getDefaultVerKey(rec, item.defaultVersion);
      ...
      variants.forEach(v=>{
        ...
        btn.style.cssText = `
          ...
          color:#fff;border:${active ? '1px solid #e50914' : '1px solid #444'};
          padding:6px 10px;border-radius:10px;font-weight:800;letter-spacing:.03em;cursor:pointer;
        `;
        btn.onclick = ()=>{
    ...
    function renderChaptersAndVariant(){
      ...
      let use;
      if (isBundle(item)){
        use = resolveBundle(item, currentRecording, currentVariant);
      } else {
        use = resolveVariant(item, currentVariant);
      }
  ...
  function playItem(item, startAt=0){
    ...
    // Fortschritt speichern (inkl. Meta)
  ...
  MJ.openPoster = function (item, fromContinue = false, originPage = 'home') {
    ...
    if (fromContinue) {
      ...
      // BUNDLE: direkt die letzte Recording/Version starten (ohne Setlist)
      if (isBundle(full)) {
        ...
        // Falls gar keine Quelle vorhanden, sichere Cleanup und Hinweis
      ...
      // Varianten-Item (kein Bundle), aber versions vorhanden → direkt starten
    ...
    if (isBundle(full) || (Array.isArray(full.chapters) && full.chapters.length > 0)) {
