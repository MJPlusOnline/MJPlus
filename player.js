...
function mountGlowBackdrop() {
  ...
  /* --- Badges (Tier) ------------------------------------------ */
    ...
    transform: scale(0.95);
      ...
      .badge.doc {
    background: linear-gradient(145deg, #b5b5b5 0%, #707070 100%);
    color: #fff !important;
  ...
  .badge.interview {
    background:#2b9df2 !important;   /* Blau */
    color:#fff !important;
  }
    .badge.performance {
    background: linear-gradient(180deg, #fff2a8 0%, #e6c85c 100%);
    color: #000 !important;
  ...
  /* --- BuyMeACoffee-Button im Player ausblenden --- */
  body.player-open #buymeacoffee {
...
`;
  ...
  if (entry.groupId) clean.groupId = entry.groupId;

  // 🆕 Zeitstempel hinzufügen, damit Continue Watching korrekt sortiert wird
  clean.updatedAt = Date.now();

  list.unshift(clean);
  saveCW(list.slice(0,40));
...
`;
  ...
  document.body.classList.add('player-open'); // 🟢 Player aktiv → Klasse setzen
}
...
function getCatalogRoot(item){
  ...
  font-size:11.5px;line-height:1.05em;text-transform:uppercase;
  font-weight:800;letter-spacing:.02em;text-align:left;
  padding:1px 0;transition:color .15s,text-shadow .15s,transform .15s;
...
`;
            ...
            el.onclick = () => {
  // --- Kapitel mit eigenem Link ---
  if (ch.link) {
    NavStack.markSetlist(true);
    hideSetlist();

    // Hilfsfunktion für Slug
    const slug = s => String(s).toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'');

    const clip = {
      id: `${item.id}::${slug(ch.label || 'clip')}`,
      groupId: item.id, // Gruppe = Collection
      title: `${item.title || 'TV'} — ${ch.label || 'Clip'}`,
      poster: item.poster || '',
      type: 'mp4',
      mp4: ch.link,
      chapters: [],
  ...
  // ALT: Kapitel im selben Stream per Startzeit
  NavStack.markSetlist(true);
  hideSetlist();
...
if (item.groupId) {
      ...
      try{
        ...
        if(d>0){
          upsertCW({
            ...
            groupId: item.groupId || (item.id.includes('::') ? item.id.split('::')[0] : null),
  ...
  function stopPlayback(){
      ...
      if(currentItem){
        ...
        if(d>0){
          upsertCW({
            ...
            groupId: currentItem.groupId || (currentItem.id.includes('::') ? currentItem.id.split('::')[0] : null),
    ...
    closeOverlay();
// 🔁 Continue-Watching-Leiste sofort aktualisieren (falls vorhanden)
try { window.MJPLUS?.updateContinueWatching?.(); } catch {}
  }
  ...
  MJ.openPoster = function (item, fromContinue = false, originPage = 'home') {
    ...
    if (fromContinue) {
  const resume = loadCW().find(x => x.id === item?.id || x.groupId === item?.id) || null;
  if (!resume) return;

  // Fallback: benutze groupId, wenn vorhanden
  const baseId = resume.groupId || resume.id;
  const full = (window.catalog || []).find(x => x.id === baseId) || item;
  const start = (resume?.t | 0) || 0;

  // Wenn Resume-Eintrag selbst eine MP4-Quelle enthält (z.B. Setlist-Clip) -> direkt abspielen
  if (resume.mp4) {
    playItem({
      id: resume.id,
      title: resume.title,
      poster: resume.poster || full.poster || '',
      mp4: resume.mp4,
  ...
  // Normales Verhalten für Bundles/Varianten
  if (isBundle(full)) {
    ...
    if (!playable.source && !playable.mp4) return;
  ...
  ${(() => {
   const t = (item.tier || '').toString().trim().toLowerCase().replace(/\s+/g, '-');
    if (!t) return '';
    let label = 'PRO';
    if (t === 'am') label = 'AM.';
    else if (t === 'tv') label = 'TV';
    else if (t === 'doc') label = 'DOC';
    else if (t === 'interview') label = 'INTERVIEW';
    else if (t === 'performance') label = 'PERFORMANCE';
    const badgeClass = t ? `badge ${t.replace(/\s+/g,'-')}` : 'badge';
...
`;
  ...
  const all = loadCW().sort((a,b)=> b.updatedAt - a.updatedAt);
  ...
  list.forEach(e=>{
    ...
    const t = (full.tier || '').toString().trim().toLowerCase().replace(/\s+/g, '-');
let label = 'PRO';
if (t === 'am') label = 'AM.';
else if (t === 'tv') label = 'TV';
else if (t === 'doc') label = 'DOC';
...
b.innerHTML = `
  ...
  ${t ? `<span class="${badgeClass}">${label}</span>` : ``}
  <div class="progress-wrap"><div class="progress" style="width:${(progress*100).toFixed(1)}%"></div></div>
`;

    b.onclick = () => {
      const openId = e.groupId || full.groupId || full.id;
      MJ.openPoster({ id: openId }, true, 'home');
    };
...
// 🟢 global verfügbar machen, damit Player.js live updaten kann
window.MJPLUS.updateContinueWatching = renderContinue;
      function renderAll(){
        if (!grid) return;
        grid.innerHTML='';
  ...
  // 🆕 alles vor 1979 -> Early Years
  if (y && y < 1979) return 'early';
...
const tourLogo = {
  ...
  special:   'sclogo.png'
    ...
    function renderTours(){
      ...
      toursSec.innerHTML = '';
      const groups = {early:[],destiny:[],triumph:[],victory:[],bad:[],dangerous:[],history:[],special:[]};
      liveItems().forEach(it => { if(!isTV(it.tier)) groups[tourKey(it)].push(it); });

      ['early','destiny','triumph','victory','bad','dangerous','history','special'].forEach(key=>{
...
/* ---------- Safe Bootstrap: initHome/initLive/initTV exactly once ---------- */
(function(){
  function safeInit(flagName, fn){
  const key = `init_${flagName}_done`;           // ✅ UNDERSCORES statt Bindestriche
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Home-Seite
    if (page === 'home' && window.MJPLUS?.initHome)
      safeInit('home', window.MJPLUS.initHome);

    // Live-Seite
    if (page === 'live' && window.MJPLUS?.initLive)
      safeInit('live', window.MJPLUS.initLive);

    // TV-Appearances-Seite
    if (page === 'tv' && window.MJPLUS?.initTV)
      safeInit('tv', window.MJPLUS.initTV);
  });
})();
...
(function(){
  ...
  MJ.initTV = function(){
    ...
    // Filter: zeigt alle Videos mit tier oder Kategorie
    const items = (window.catalog || []).filter(i => {
      const tier = String(i.tier || '').trim().toLowerCase();
      const cats = Array.isArray(i.category)
        ? i.category.map(c => String(c).toLowerCase())
        : [String(i.category || '').toLowerCase()];
      return ['tv','doc','interview','performance'].includes(tier)
        || cats.includes('tv appearances')
        || cats.includes('tv appearance')
        || cats.includes('interview');
    });
...
items.forEach(item => {
  ...
  const tier = (item.tier || '').toString().trim().toLowerCase().replace(/\s+/g, '-');
  let label = 'PRO';
  if (tier === 'am') label = 'AM.';
  else if (tier === 'tv') label = 'TV';
  else if (tier === 'doc') label = 'DOC';
  else if (tier === 'interview') label = 'INTERVIEW';
  else if (tier === 'performance') label = 'PERFORMANCE';
  const badgeClass = tier ? `badge ${tier}` : 'badge';

  b.innerHTML = `
    <img src="${item.poster || ''}" alt="${item.title || ''}" onerror="this.style.opacity=.2">
    ${tier ? `<span class="${badgeClass}">${label}</span>` : ``}
  `;

  b.onclick = () => MJ.openPoster(item, false, 'tv');
  b.addEventListener('keydown', e => {
    if (['Enter', 'OK', 'Select', ' '].includes(e.key)) {
      e.preventDefault();
      b.click();
    }
  });
