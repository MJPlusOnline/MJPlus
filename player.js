...
(() => {
  ...
  function muxMp4FromM3u8(u){
    // Aus https://stream.mux.com/<id>.m3u8 -> https://stream.mux.com/<id>/medium.mp4
    try{
  ...
  function playItem(item, startAt=0){
    ...
    const tryFallbackToMp4 = ()=>{
      ...
      if (!mp4Url){ return; } // kein MP4 verfügbar
      fellBack = true;
    ...
    // Primäre Wahl: HLS (hls.js) oder native HLS
    ...
    if (hasHlsJs){
      ...
      // Direkt MP4 (TV-Browser wie Amazon Silk)
      const mp4Url = item.mp4 || muxMp4FromM3u8(item.source);
      if (!mp4Url){
        alert('This browser cannot play HLS and no MP4 fallback is available.');
        return;
      }
      startVideoWith(mp4Url, 'mp4');
    }
    ...
    // Wenn das Video selbst einen Fehler wirft (z.B. M3U8 unsupported), fallbacken
    videoEl.addEventListener('error', tryFallbackToMp4, {once:true});

    // Fortschritt speichern (Continue Watching)
