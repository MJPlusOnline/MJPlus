...
(() => {
  ...
  /* ---------- Varianten-Helpers (für Original/Enhanced) ---------- */
  ...
  // Liefert ein „spielbares“ Objekt (id/title/poster/type/source/mp4/chapters)
  function resolveVariant(item, currentKey){
    // Neues Schema: versions[]
    if (Array.isArray(item.versions) && item.versions.length){
      let key = currentKey || item.defaultVersion || item.versions[0]?.key || 'orig';
    ...
    // Altes Schema: variants.{original,enhanced}
  ...
  /* ---------- Setlist anzeigen (mit Variantenschalter) ---------- */
  function showSetlist(item){
    ...
    // aktive Variante festlegen, falls vorhanden
    let currentVariant = null;
    if (hasVariants(item)) {
      currentVariant = item.defaultVersion
        || (item.variants && item.variants.original ? 'original'
            : (item.variants && item.variants.enhanced ? 'enhanced' : null));
    }

    // alten Variantenschalter entfernen (falls schon vorhanden)
    ...
    // Variantenschalter rechts im Header
    if (hasVariants(item)) {
      const switchWrap = document.createElement('div');
      switchWrap.id = 'variantSwitch';
      switchWrap.style.cssText = 'margin-left:auto;display:flex;gap:8px;';

      const mkBtn = (key, label, enabled) => {
        const b = document.createElement('button');
        b.type = 'button';
        b.textContent = label;
        b.disabled = !enabled;
        b.style.cssText = `
          background:${currentVariant===key ? '#e50914' : '#000'};
          color:#fff;border:${currentVariant===key ? '1px solid #e50914' : '1px solid #ffffff33'};
          padding:8px 12px;border-radius:10px;font-weight:800;letter-spacing:.03em;cursor:${enabled?'pointer':'default'};
          opacity:${enabled? '1' : '.5'};
          transition:filter .15s, transform .15s, background .15s, border-color .15s;
        `;
        if (enabled) {
          b.onmouseover = ()=>{ if(currentVariant!==key){ b.style.borderColor='#ffffff55'; } };
          b.onmouseout  = ()=>{ if(currentVariant!==key){ b.style.borderColor='#ffffff33'; } };
          b.onclick = ()=>{
            if (currentVariant !== key) {
              currentVariant = key;
              Array.from(switchWrap.children).forEach(btn=>{
                const isOrigBtn = btn.textContent.toLowerCase().includes('original');
                const isEnhBtn  = btn.textContent.toLowerCase().includes('enhanced');
                const isActive  = (isOrigBtn && currentVariant==='original') || (isEnhBtn && currentVariant==='enhanced');
                btn.style.background = isActive ? '#e50914' : '#000';
                btn.style.borderColor = isActive ? '#e50914' : '#ffffff33';
              });
              renderChapters();
            }
          };
      ...
      const hasOrig = !!(item.versions ? item.versions.some(v=>v.key==='original') : item.variants?.original);
      const hasEnh  = !!(item.versions ? item.versions.some(v=>v.key==='enhanced') : item.variants?.enhanced);

      switchWrap.appendChild(mkBtn('original','Original',hasOrig));
      switchWrap.appendChild(mkBtn('enhanced','Enhanced',hasEnh));

      const header = setlistModal.querySelector('.setlist-header') || document.querySelector('.setlist-header');
      if (header) header.appendChild(switchWrap);
    }

    // Kapitel rendern (in 2 Spalten)
    const renderChapters = ()=>{
      setlistCols[0].innerHTML=''; setlistCols[1].innerHTML='';
      const use = resolveVariant(item, currentVariant);
      ...
      const renderCol = (list, colIndex, target) => {
        list.forEach((ch, i)=>{
          ...
          btn.onclick = ()=>{
            hideSetlist();
            const toPlay = resolveVariant(item, currentVariant);
            const startAt = Number(ch.sec||0);
            playItem(toPlay, startAt);
          };
  ...
  MJ.openPoster = function (item, fromContinue = false, originPage = 'home') {
    ...
    if (fromContinue) {
      ...
      showSetlist(full.chapters?.length ? full : null); // falls Kapitel existieren, erst Setlist
      if (!full.chapters || !full.chapters.length) playItem(full, start);
      return;
    }
