...
(() => {
  ...
  function ensureOverlay(){
    ...
    // SETLIST MODAL (zweispaltig, Look wie vorher: white, hover red)
    ...
    // Body: 2 Spalten, weiße Schrift, Hover rot (wie vorher)
    ...
    // DPAD/Keyboard in der Setlist (inkl. Left/Right zwischen Spalten)
  ...
  /* ---------- Setlist anzeigen (2-reihig, Look wie vorher) ---------- */
  function showSetlist(item){
    ...
    const render = (list, colIndex, target) => {
      list.forEach((ch, i)=>{
        ...
        // LOOK: weiß, bei Hover/Focus rot – wie vorher
  ...
  /* ---------- HLS Playback (Safari-freundlich) ---------- */
  function cleanHls(){ if(hls){ try{hls.destroy();}catch{} hls=null; } }

  function attachSrc(url){
    cleanHls();

    const canNativeHLS = !!videoEl.canPlayType && videoEl.canPlayType('application/vnd.apple.mpegurl');
    const isSafari = /^((?!chrome|android).)*safari/i.test(navigator.userAgent);

    if (canNativeHLS || isSafari) {
      // Native HLS (Safari / iOS)
      videoEl.src = url;
      videoEl.load(); // wichtig für iOS
      return;
    }

    if (window.Hls && Hls.isSupported()){
      hls = new Hls({
        // WICHTIG: kein overrideNative -> Safari bleibt nativ
      ...
      hls.on(Hls.Events.ERROR, (_e,d)=>{ if(d?.fatal){ try{hls.recoverMediaError();}catch{} }});
      return;
    }
  ...
  function playItem(item, startAt=0){
    ...
    // versuchen zu spielen, sobald möglich (iOS gibt play() erst nach canplay frei)
    const tryPlay = () => { try { videoEl.play().catch(()=>{}); } catch {} };

    // Quelle setzen (nutzt nativ auf Safari)
    attachSrc(item.source);

    // Startposition erst nach Metadata setzen
    videoEl.addEventListener('loadedmetadata', ()=>{
      try { if (startAt>0) videoEl.currentTime = startAt; } catch {}
    }, {once:true});

    // Abspielen, wenn bereit
  ...
  document.addEventListener('keydown',(e)=>{
    ...
    if (sm && sm.style.display==='flex') return;  // wenn Suche offen, hier nichts
    if (isSetlistOpen()) return;                   // Setlist fängt selbst ab
  ...
  MJ.openPoster = function (item, fromContinue = false, originPage = 'home') {
    ...
    if (fromContinue) {
      ...
      // Continue Watching => direkt spielen, KEINE Setlist
      playItem(full, start);
      return;
    }

    // Katalog-Klick: Setlist wenn Kapitel vorhanden, sonst direkt
  ...
  MJ.initHome = function(){
    try{
      ...
      function renderContinue(){
        ...
        list.forEach(e=>{
          // Katalogeintrag mergen (für poster/source etc.)
          ...
          // Browser-freundlicher Click (keine Setlist)
          b.onclick = ()=>{
            MJ.openPoster({ id: full.id }, true, 'home');  // fromContinue=true => direkt Resume
          };
          b.addEventListener('keyup', (ke)=>{
            if(ke.key==='Enter' || ke.key===' ' || ke.key==='OK' || ke.key==='Select') b.click();
          });

          continueRow.appendChild(b);
        });

        // Keine Container-Delegation
  ...
  /* ---------- (Optional) SEARCH UI: wird nur aktiv, wenn HTML-Elemente existieren ---------- */
  MJ.initSearchUI = function(){
    ...
    const closeB = document.getElementById('searchClose');
    ...
    const renderResults = (q) => {
      ...
      const results = !qq ? [] : window.catalog.filter(i=>{
    ...
    function open(){ modal.style.display='flex'; setTimeout(()=> input?.focus(), 30); }
    function close(){ modal.style.display='none'; }

    openBtn.addEventListener('click', open);
    closeB?.addEventListener('click', close);
    xBtn?.addEventListener('click', ()=>{ input.value=''; close(); });
    ...
    document.addEventListener('backbutton', function (e) {
      if (modal.style.display==='flex'){ e.preventDefault(); close(); }
    }, false);
  };
